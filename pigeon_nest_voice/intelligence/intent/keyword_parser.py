...
_QUESTION_WORDS = re.compile(
    r"^(?:什么|啥|哪个|哪些|哪种|多少|几个|谁|怎么|为什么|"
    r"什么东西|什么玩意|什么意思|啥意思|啥东西)[？?吗呢啊呀吧的]*$"
)
    ...
    async def parse(self, text: str) -> Intent:
        ...
        for rule in self._rules:
            ...
            if rule.action == "control_device":
                ...
                if value:
                    value = value.strip()
                    # 疑问词不是有效的物品/参数，跳过此规则走对话
                    if _QUESTION_WORDS.match(value):
                        logger.debug("跳过规则 %s: 提取到疑问词 '%s'",
                                     rule.action, value)
                        continue
                    params[rule.param_extractor] = value
